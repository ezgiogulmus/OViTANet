...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    # def get_closest_downsample(self, target_downsample):
        
    #     tf = tif.TiffFile(self.full_path)
    #     w, h = self.wsi.level_dimensions[0]
    #     assert w == tf.pages[0].shape[1] and h == tf.pages[0].shape[0], "TiffFile and OpenSlide does not produce same image."
    #     min_diff = None
    #     for p in reversed(list(tf.pages)):
    #         page_h, page_w = p.shape[:2]
    #         if int(w/page_w) == int(h/page_h):
    #             if min_diff is None:
    #                 min_diff = abs(target_downsample - w/page_w)
    #                 closest_target = w/page_w
    #                 img = p.asarray()
    #                 self.selected_page = p
    #             if abs(target_downsample - w/page_w) < min_diff:
    #                 min_diff = abs(target_downsample - w/page_w)
    #                 closest_target = w/page_w
    #                 img = p.asarray()
    #                 self.selected_page = p
    #                 break
    #     self.level_downsamples = [(closest_target, closest_target)]
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, patch_downsample[0], patch_downsample[1])
