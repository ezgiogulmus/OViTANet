...
if __name__ == '__main__':
    ...
    # print(tab_df.shape)
    gen_data = np.unique([i.split("_")[-1] for i in tabular_cols if i.split("_")[-1] in ["rna", "dna", "mut", "cnv"]])
    if len(gen_data) > 0:
        for g in gen_data:
    ...
    model, _, _ =  init_model(run_args, os.path.join(args.load_dir, args.ckpt_name))
    model.eval()
    ...
    for i in range(len(process_stack)):
        ...
        tab_data = slide_data.loc[slide_data['case_id'] == slide_name[:12], tabular_cols].values
