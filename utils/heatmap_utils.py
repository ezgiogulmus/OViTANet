...
def compute_from_patches(wsi_object, tab_features, model=None, feature_extractor=None, batch_size=512,  
    ...
    for idx, (roi, coords, _) in enumerate(roi_loader):
        ...
        with torch.no_grad():
            ...
            _, A_scores = model(x_path=features, x_tabular=tab_features, return_feats=False, return_weights=True)
            A = torch.mean(A_scores, dim=1).cpu().numpy()
            A = A[0, -1, 1:-1]
            if ref_scores is not None:
                for score_idx in range(len(A)):
                    A[score_idx] = score2percentile(A[score_idx], np.squeeze(ref_scores))
            A_list.extend(A)
...
def get_attention_scores(model, img_features, tab_features):
    ...
    tab_features = tab_features.to(device)
    with torch.no_grad():
        logits, A = model(x_path=img_features, x_tabular=tab_features, return_feats=False, return_weights=True)
        A_scores = torch.mean(A, dim=1).cpu().numpy()
        A_scores = A_scores[0, -1]
