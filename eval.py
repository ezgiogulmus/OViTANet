...
def main(eval_args): 
	...
	eval_args.results_dir = os.path.dirname(eval_args.load_from)
	data_root_dir = eval_args.data_root_dir
	...
	if f'{args.dataname}_cv{args.cv}_features.csv' in os.listdir(eval_args.results_dir):
	...
	if test_all:
		dataset = surv_dataset.return_splits(return_all=True, stats_path=os.path.join(eval_args.results_dir, f'train_stats_{args.cv}.csv'))
	else:
		assert args.dataname in args.split_dir, "Testing is only possible for the same dataset."
		_, _, dataset = surv_dataset.return_splits(args.cv)
	
	result_latest = eval_model(dataset, eval_args.results_dir, args)
